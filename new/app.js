...
localStorage.setItem('shmSession', JSON.stringify({ user: u, role: ud.role, name: ud.name }));        document.getElementById('authScreen').classList.remove('visible');
  ...
  applyPermissions() {
    ...
    async delDef(type, id) {
        ...
        const path = pathMap[type] || `jawaher_def/${type}`;
    ...
    async addCustomColorDef() {
        const nameEl = document.getElementById('defColorName');
        const hexEl  = document.getElementById('defColorHex');
        const name = nameEl?.value.trim();
        const hex  = hexEl?.value || '#000000';
    ...
    async deleteCustomColor(firebaseKey, colorName) {
        // Validation: تحقق من عدم وجود رصيد مرتبط بهذا اللون
        const usedIn = [];
        ...
        if (usedIn.length > 0) {
            this.toast(`⚠ لا يمكن حذف اللون "${colorName}" — مستخدم في: ${usedIn.slice(0,3).join('، ')}${usedIn.length > 3 ? '...' : ''}`, 'error');
            return;
        }
        ...
        container.innerHTML = this.customColors.map(c => `
            <div style="display:flex;align-items:center;justify-content:space-between;padding:.55rem 0;border-bottom:1px solid var(--border-2)">
                ...
                <div style="display:flex;gap:.4rem">
                    ...
                    <button class="btn-j btn-ruby btn-xs-j" onclick="app.deleteCustomColor('${c._key}','${c.name}')" title="حذف اللون">
...
    },
    ...
    // ============ PRINT ============
      printOrder(o, id) {
        ...
        win.document.close();
        // window.onload in label HTML triggers print after barcode renders
    ...
    executePrint(ids) {
        ...
        win.document.close();
        // window.onload in label HTML triggers print after all barcodes render
        const updates = {};
        setTimeout(() => {
...
<button class="btn-j btn-gold btn-xs-j" onclick="app.openOrderModal('${id}')"><i class="fas fa-eye"></i></button>
    ...
    async rBulkDelete() {
        ...
        // إرجاع المخزون لكل طلب مخصوم قبل الحذف
        for (const id of this.selectedR) {
            await this._returnStock(id);
        }
        const upd = {};
    ...
    renderNimSizesGrid() {
        ...
        // الباركود الأساسي من حقل الباركود الرئيسي للمنتج
        ...
        grid.innerHTML = (this.nimSizeRows || []).map((row, i) => {
            ...
            // القيمة الافتراضية للباركود: ما أدخله المستخدم مسبقاً → وإلا باركود المنتج الأساسي
            const b = row.barcode !== undefined ? row.barcode : mainBarcode;
            const bPlaceholder = mainBarcode || 'باركود (اختياري)';
            return `<div class="col-12 nim-size-row" id="nimsr_${i}">
                <div style="display:flex;gap:4px;align-items:center;flex-wrap:wrap;background:var(--paper);padding:8px;border-radius:8px;border:1px solid var(--border);margin-bottom:4px">
                    ...
                    <input type="text"   class="form-control-j nim-b-val" placeholder="${bPlaceholder}"
                        value="${b}" style="flex:1;min-width:100px;font-size:.85rem;font-family:monospace;background:${b && b === mainBarcode ? 'rgba(201,168,76,.06)' : 'inherit'}"
                        dir="ltr" title="يرث باركود المنتج تلقائياً — يمكن تعديله">
    ...
    addNimSizeRow() {
        ...
        // المقاس الجديد يرث الباركود الأساسي تلقائياً كقيمة قابلة للتعديل
        const mainBarcode = document.getElementById('nimBarcode')?.value?.trim().toUpperCase() || '';
        this.nimSizeRows.push({ size: '', barcode: mainBarcode });
        this.renderNimSizesGrid();
    },
    removeNimSizeRow(i) {
                ...
                <div class="item-card" style="border-top:4px solid ${colorBorder}">
                    ...
                    <div class="item-card-body">
                        ...
                        <div style="display:flex;gap:.5rem;flex-wrap:wrap;margin-bottom:.5rem">
                            ${w.buyPrice ? `<div style="background:var(--paper-warm);border:1px solid var(--border);border-radius:8px;padding:4px 10px;font-size:.74rem">
                                <span style="color:var(--ink-mid)">شراء</span> <strong style="color:var(--ink)">${w.buyPrice} JOD</strong>
                            </div>` : ''}
                            ${w.sellPrice ? `<div style="background:rgba(26,107,74,.08);border:1px solid rgba(26,107,74,.25);border-radius:8px;padding:4px 10px;font-size:.74rem">
                                <span style="color:var(--emerald)">بيع</span> <strong style="color:var(--emerald);font-size:.82rem">${w.sellPrice} JOD</strong>
                            </div>` : ''}
                        </div>` : ''}
                        ${w.notes ? `<div style="background:rgba(201,168,76,.06);border-right:3px solid var(--gold);border-radius:0 8px 8px 0;padding:5px 10px;font-size:.76rem;color:var(--ink-mid);margin-bottom:.5rem;line-height:1.4">
                            <i class="fas fa-sticky-note" style="color:var(--gold);margin-left:4px"></i>${w.notes}
                        </div>` : ''}
 ...
 async saveReturn() {
   ...
   adjustQty(id, delta) {
    ...
    async deleteItemSize(itemId, sizeKey) {
        const item = this.warehouse[itemId]; if (!item) return;
        const dispKey = sizeKey.includes(' - ') ? sizeKey : sizeKey;
        if (!confirm(`هل أنت متأكد من حذف المقاس "${dispKey}" نهائياً من المستودع؟\nسيتم حذف الرصيد المرتبط به أيضاً.`)) return;
        const updates = {};
        updates[`jawaher_warehouse/${itemId}/sizes/${sizeKey}`] = null;
        if (item.variations && item.variations[sizeKey] !== undefined)
            updates[`jawaher_warehouse/${itemId}/variations/${sizeKey}`] = null;
        if (item.sizeColors && item.sizeColors[sizeKey] !== undefined)
            updates[`jawaher_warehouse/${itemId}/sizeColors/${sizeKey}`] = null;
        await update(ref(db), updates);
        this.log('stock', itemId, `حذف مقاس/لون: ${sizeKey}`);
        this.toast(`تم حذف المقاس "${dispKey}" ✓`, 'success');
        // إعادة فتح المودال لتحديث الأرقام
    ...
    setTimeout(() => {
        ...
        const labelsHtml = labels.map(({ id, o }, labelIndex) => {
            ...
            const items = o.items || [{
                ...
                size: o.size, qty: o.qty, price: o.price
            }];

            // باركود مميز لكل ملصق
            const bcId = `bc_lbl_${labelIndex}_${id.slice(-6)}`;
            const bcValue = id.slice(-12);
            barcodeScripts += `try { JsBarcode("#${bcId}", "${bcValue}", { format:"CODE128", width:1.4, height:28, displayValue:true, fontSize:9, margin:2, background:"transparent" }); } catch(e) { console.warn("barcode error", e); }`;

            // سعر البيع من المستودع
            ...
            const contactIcons = {
                ...
                'انستا':    { svg: `<svg width="11" height="11" viewBox="0 0 24 24"><path fill="#d6249f" d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069z"/></svg>`, label:'انستغرام' },
            ...
            const contactHtml = chInfo
                ? `<span style="display:inline-flex;align-items:center;gap:2px;font-size:.5rem;font-weight:700">${chInfo.svg} ${chInfo.label}</span>`
                : '';
            const weightHtml = (o.weightKg || o.lengthCm) ? `
                <div style="font-size:.48rem;color:#666;margin-top:1px">
                    ${o.weightKg ? `⚖ ${o.weightKg}kg` : ''} ${o.lengthCm ? `📏 ${o.lengthCm}cm` : ''}
                </div>` : '';
            ...
            return `<div class="label-page">
                <div class="label-inner">
                    ...
                    <div class="lbl-header">
                        ...
                        <div class="lbl-subheader">
                            <span>📞 077 65 01 333</span>
                            <span>👤 ${o.entryUser || ''}</span>
                        </div>
                    </div>
                    ...
                    <div class="lbl-body">
                        ...
                        <div class="lbl-col lbl-col-right">
                            <div class="lbl-field">
                                <div class="lbl-field-label">اسم الزبون</div>
                                <div class="lbl-field-val lbl-name">${o.custName || ''}</div>
                            </div>
                            <div class="lbl-field lbl-field-sm">
                                <div class="lbl-field-label">التواصل</div>
                                <div class="lbl-field-val">${contactHtml || '—'}</div>
                            </div>
                            <div class="lbl-field">
                                <div class="lbl-field-label">الصنف</div>
                                <div class="lbl-field-val lbl-item">${items.map(it => it.itemName||'').join(' ، ')}</div>
                            </div>
                            <div class="lbl-field lbl-field-sm">
                                <div class="lbl-field-label">اللون</div>
                                <div class="lbl-field-val">${items.map(it=>it.itemColor||'').join('،')||'—'}</div>
                            </div>
                            <div class="lbl-field lbl-field-sm">
                                <div class="lbl-field-label">المقاس</div>
                                <div class="lbl-field-val">${items.map(it=>it.size||'').filter(Boolean).join('،')||'—'}</div>
                            </div>
                            ${sellPriceVal ? `<div class="lbl-field lbl-field-sm">
                                <div class="lbl-field-label">سعر القطعة</div>
                                <div class="lbl-field-val" style="color:#1A6B4A;font-weight:700">${sellPriceVal} JOD</div>
                            </div>` : ''}
                            ${weightHtml}
                        </div>

                        <!-- LEFT col: delivery info -->
                        <div class="lbl-col lbl-col-left">
                            <div class="lbl-field">
                                <div class="lbl-field-label">عنوان الزبون</div>
                                <div class="lbl-field-val lbl-addr">${o.governorate ? o.governorate + ' - ' : ''}${o.custAddr || '—'}</div>
                            </div>
                            <div class="lbl-field">
                                <div class="lbl-field-label">رقم الهاتف</div>
                                <div class="lbl-field-val lbl-phone" dir="ltr">${o.custMob || ''}</div>
                            </div>
                            <div class="lbl-field lbl-price-box">
                                <div class="lbl-field-label" style="color:#1A6B4A">القيمة شامل</div>
                                <div class="lbl-field-val lbl-price">${o.price || 0} JOD</div>
                            </div>
                            <div class="lbl-field" style="flex:1">
                                <div class="lbl-field-label">ملاحظات</div>
                                <div class="lbl-field-val lbl-notes">${o.tags || ''}</div>
                            </div>
        ...
        const css = `
            ...
            * { box-sizing: border-box; margin: 0; padding: 0; }
            ...
            .label-page {
                ...
                padding: 2.5mm; overflow: hidden;
            }
            .label-inner {
                width: 100%; display: flex; flex-direction: column;
                border: 2px solid #1A3A8F; border-radius: 5px; overflow: hidden;
            }
            .lbl-header {
                background: #0F2260; color: #fff;
                padding: 2.5px 6px; border-bottom: 2px solid #1A3A8F;
                flex-shrink: 0;
            }
            ...
            .lbl-field {
                background: #f4f6ff; border: 1px solid #dde;
                border-radius: 3px; padding: 2px 5px; overflow: hidden;
            }
            ...
            .lbl-warning {
                background: #FFF0F0; border: 1.5px solid #C02525;
                border-radius: 3px; padding: 2px 5px;
                font-size: 6pt; font-weight: 800; color: #C02525;
                text-align: center; flex-shrink: 0;
            }
            .lbl-barcode-wrap {
                text-align: center; padding: 1px 4px 2px;
                border-top: 1px solid #dde; flex-shrink: 0;
