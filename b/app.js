...
window.app = {
  ...
  applyPermissions() {
    ...
    initPremiumLanding() {
        if (this._premiumLandingReady) return;
        this._premiumLandingReady = true;
        const focusLogin = event => {
            event?.preventDefault?.();
            document.getElementById(`authLoginPanel`)?.scrollIntoView({ behavior: `smooth`, block: `center` });
            setTimeout(() => document.getElementById(`loginUser`)?.focus(), 420);
        };
        document.querySelectorAll(`.premium-contact-btn, .premium-nav-cta`).forEach(el => {
            el.addEventListener(`click`, focusLogin);
        });
    },
